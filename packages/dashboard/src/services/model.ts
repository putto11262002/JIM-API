import { EncodedModelGetQuery, Model, ModelCreateInput, ModelExperienceCreateInput, ModelUpdateInput, PaginatedData } from "@jimmodel/shared";
import { GenericAbortSignal } from "axios";
import axiosClient from "../lib/axios";

async function getAll(query?: EncodedModelGetQuery, signal?: GenericAbortSignal){
...
async function addImage(modelId: string, modelImageCreateInput: {image: File, type: string}){
    const formData = new FormData()
    formData.append("image", modelImageCreateInput.image)
      formData.append("type", modelImageCreateInput.type)
    await axiosClient.post(`/models/${modelId}/images`, formData, {headers: {"Content-Type": "multipart/form-data"}})
    
}


async function addExperience(modelId: string, experience: ModelExperienceCreateInput | ModelExperienceCreateInput[]){
